<?php
  ...
  $enf_solution = utf8_decode($_POST['editnfe_Solution']);

  $data = $db->prepare("UPDATE man_manager SET status=?, nfe_modelo=?, nfe_todo=?, nfe_id=?, nfe_nv=?, nfe_nf=?, problem=?, solution=? WHERE entryid=?");

  if (!empty($enf_id) || !empty($enf_status) || !empty($enf_procedure) || !empty($enf_model)){
    $data->execute([$enf_status, $enf_model, $enf_procedure, $enf_id, $enf_nv, $enf_nf, $enf_problem, $enf_solution, $enf_gid]) or die(print_r($db->errorInfo(), true));
