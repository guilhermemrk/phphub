<?php
  ...
  include './../db/connect.php';
    ...
    $data = $db->prepare("INSERT INTO man_manager (companyid, status, nfe_modelo, nfe_todo, nfe_id, nfe_nv, nfe_nf, problem, addedBy, mangroup) VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    if (!empty($anfe_company) || !empty($anfe_status) || !empty($anfe_model) || !empty($anfe_id) || !empty($anfe_nv) || !empty($anfe_nf) || !empty($anfe_procedure) || !empty($sUsername)){
      $data->execute([$anfe_company, $anfe_status, $anfe_model, $anfe_procedure, $anfe_id, $anfe_nv, $anfe_nf, $anfe_problem, $sUsername, $sUsergroup]) or die(print_r($db->errorInfo(), true));
